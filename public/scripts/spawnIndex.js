...
async function loadCards(type) {
    const endpoint = type === 'food' ? '/api/dishes' : '/api/drinks';
    const response = await fetch(endpoint);
    const items = await response.json();

    const container = document.querySelector(`.scrollable.${type}`);
    container.innerHTML = ''; // Очистка перед добавлением

    items.forEach(item => {
        const card = document.createElement('div');
        card.classList.add('card');
        card.innerHTML = `
            <img src="${item.image}" alt="${item.name}" class="cardPhoto">
            <div class="text">
...
async function loadTranslations() {
    ...
    const currentLang = document.querySelector('.selected-language').textContent.toLowerCase();
